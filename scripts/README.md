...
| `build_award_frontend_mapping.py` | Builds the Award UI → database mapping from JSP/tag files plus the field dictionary. |

### Award graph generation

`build_award_graph.py` has been **retired** — it was a single-module implementation of
what `build_object_graph.py` now does generically, and reproduced the same output. Its
history remains in Git. Award graph generation is now:
...
Add a new module by adding an entry to `MODULES` in `build_object_graph.py`.
