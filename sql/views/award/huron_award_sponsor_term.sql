WITH huron_award_version AS (
    SELECT award_id, award_number, sequence_number, selection_rule
    FROM (
        SELECT a.award_id,
               a.award_number,
               a.sequence_number,
               CASE WHEN a.award_sequence_status = 'ACTIVE'
                    THEN 'ACTIVE_STATUS' ELSE 'MAX_SEQUENCE_FALLBACK' END AS selection_rule,
               ROW_NUMBER() OVER (
                   PARTITION BY a.award_number
                   ORDER BY CASE WHEN a.award_sequence_status = 'ACTIVE' THEN 0 ELSE 1 END,
                            a.sequence_number DESC,
                            a.award_id        DESC
               ) AS rn
        FROM   kcoeus.award a
    )
    WHERE rn = 1
)
SELECT
       st.award_sponsor_term_id            AS award_sponsor_term_id,
       st.award_id                         AS award_id,
       st.award_number                     AS award_number,
       st.sequence_number                  AS sequence_number,
       st.sponsor_term_id                  AS sponsor_term_id,
       t.sponsor_term_code                 AS sponsor_term_code,
       t.description                       AS sponsor_term_description,
       t.sponsor_term_type_code            AS sponsor_term_type_code,
       tt.description                      AS sponsor_term_type_description,
       st.ver_nbr                          AS version_number,
       st.update_timestamp                 AS update_timestamp,
       st.update_user                      AS update_user
FROM       kcoeus.award_sponsor_term st
LEFT JOIN  kcoeus.sponsor_term      t  ON t.sponsor_term_id        = st.sponsor_term_id
LEFT JOIN  kcoeus.sponsor_term_type tt ON tt.sponsor_term_type_code = t.sponsor_term_type_code
JOIN       huron_award_version v ON v.award_id = st.award_id

-- AWARD_SPONSOR_TERM stores only the surrogate SPONSOR_TERM_ID; without the
-- SPONSOR_TERM join the dataset carries no readable term text at all.

-- Award root population rule: one row per AWARD_NUMBER, the version KC marks
-- ACTIVE, falling back to the highest sequence where no ACTIVE row exists
-- (202 award numbers). Verified: 43,202 selected rows for 43,202 award numbers.
-- Children are retrieved through the SELECTED root's AWARD_ID -- MAX(SEQUENCE_NUMBER)
-- is never recomputed on a child table, which would risk mixing versions.
-- Remove the huron_award_version join to expose all historical sequences.
